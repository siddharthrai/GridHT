 ...
 * @invariant $none
 */
public abstract class AllocPolicy extends GridSimCore
{
     ...
     * @post $result != null
     */
    public gridmonitor.Accumulator getTotalLoad() {
        //Accumulator acc=accTotalLoad_;
        //System.out.println("%%%%"+accTotalLoad_.getSum());
     ...
     * @post $none
     */
    public void init(ResourceCharacteristics res, gridmonitor.ResourceCalendar cal,
                     Sim_port port)
    {
        ...
        //int simTimeZone = calendar.getTimeZone().getRawOffset() /AdvanceReservation.HOUR;

        // then convert into the local resource time
        //initTime_ = AdvanceReservation.convertTimeZone( simTime, simTimeZone,resource_.getResourceTimeZone() );
    }
     ...
     * @post $none
     */
    protected AllocPolicy(String resName, String entityName,int gridmonitoradminid_,int nodeid_) throws Exception
    {
    ...
    protected void addTotalLoad(double load) {
        ...
        if((accTotalLoad_.add(load))==0)
        {
            ...
            this.send(this.myId_,GridMonitorTags.SCHEDULE_NOW,GridMonitorTags.UPDATE_INDEX,new GridMonitorIO(this.myId_,this.myId_,(Object)load));
             //indexCurrentLoad();
        }
         //else
         {
     ...
     * child class, if the below algorithm doesn't suitable for a particular
     * type of scheduling:
     * <code>
     * <br> <br>
